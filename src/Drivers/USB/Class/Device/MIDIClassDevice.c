...
uint8_t MIDI_Device_Flush(USB_ClassInfo_MIDI_Device_t* const MIDIInterfaceInfo)
{
	...
	if (/*Endpoint_BytesInEndpoint()*/usb_data_buffer_IN_index)//TODO is this better? CJB 2013
	{
		Endpoint_ClearIN();
                                    ...
                                    MIDI_EventPacket_t* const Event)
{
	...
	if (Endpoint_Read_Stream_LE(Event, sizeof(MIDI_EventPacket_t), NULL) == ENDPOINT_RWSTREAM_IncompleteTransfer)
		return false;

	if (!(Endpoint_IsReadWriteAllowed()))//More than one packet received? Or is this why it's sending out? Is an interrupt changing the value?
	  Endpoint_ClearOUT();
